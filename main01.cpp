...
int state_global, button_global;
...
{
  //Store the x and y coordinates when mouse is clicked,
  //also store the clicked button and state
  if(state == GLUT_DOWN){    
    mouse_old_x = x;
    mouse_old_y = y;
    state_global = state;
...
void mouseMotion(int x, int y)
{
  ...
  if(button_global == GLUT_LEFT_BUTTON && state_global == GLUT_DOWN){    
    rotate_x = (x - mouse_old_x);
    rotate_y = (y - mouse_old_y);
  }
  
  //If right button is clicked
  if(button_global == GLUT_RIGHT_BUTTON && state_global == GLUT_DOWN){    
    translate_z = (y - mouse_old_y)/50;  //Divide by a constant to adjust the zooming speed
  }
  
  //If middle button is clicked
  if(button_global == GLUT_MIDDLE_BUTTON && state_global == GLUT_DOWN){ 
    move_x = (x - mouse_old_x)/60;    //Divide by a constant to adjust the moving speed
    move_y = -(y - mouse_old_y)/60;
  }
